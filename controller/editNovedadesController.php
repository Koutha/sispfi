...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {//Validacion de session iniciada
    if ($now > $_SESSION['expire']) { //expiro el tiempo maximo de session
        ...
        require_once('view/loginView.php');   
    }elseif (isset($_POST['submit'])&&$_POST['submit']=='editNovedades'){//validacion de session correcta - entrada via post desde formulario de registro
            require_once('models/novedadesModel.php');
            $novObject = new novedadesClass();
            $id = $_POST['id'];
            
            $novObject->setId_novedades($id);
            ...
            $novObject->updateNovedades();
            $novObject->delRoboCerdos();
            ...
            $_SESSION['editNovedades'] = 1 ;
            
            // var_dump($_POST);
            // var_dump($novObject);
            // $novedad = $novObject->getBy($id); //novedad a modificar
            // require_once('view/editNovedadesView.php');
            header('Location:?do=editNovedades&id='.$id);
            ...
            if (isset($_GET['id'])){
                ...
                $novedad = $novObject->getBy($id); //novedad a modificar
                // print_r($novedad);
                // echo date_format(date_create($novedad[0]['fecha_hecho']), 'd/m/Y h:i A');
            }
            require_once('view/editNovedadesView.php');
        }
