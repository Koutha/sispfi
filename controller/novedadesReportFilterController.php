...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {//Validacion de session iniciada
	...
	}elseif (isset($_POST['submit'])&&$_POST['submit']=='novedadesReportFilter'){//validacion de session correcta - entrada via post desde formulario de registro
		require_once('models/novedadesModel.php');
		$novObject = new novedadesClass();
		$inicio = $_POST['fecha_inicio'].'/01';
		$fin = (new DateTime($_POST['fecha_fin'].'/01'))->modify('last day of this month');
		$fechaFin = $fin->format('Y/m/d');
		$lugar = $_POST['lugar'];
		//----para obtener los meses del intervalo seleccionado
		$start    = (new DateTime($inicio))->modify('first day of this month');
		$end      = (new DateTime($fechaFin))->modify('first day of next month');
		$interval = DateInterval::createFromDateString('1 month');
		$period   = new DatePeriod($start, $interval, $end);
		foreach ($period as $dt) {
			// echo $formatted_time = strftime("%m - %Y", $dt->getTimestamp()). "<br>\n";
			$mes = strftime("%m", $dt->getTimestamp());
			$año = strftime("%Y", $dt->getTimestamp());
			$totalYear1[] = $novObject->totalMesAñoFilter($inicio,$fechaFin,$lugar,$mes,$año);
			$ejeX[] = strftime("%B - %Y", $dt->getTimestamp()); //arreglo con cada mes - año del intervalo
		}
		$total1 = $novObject->totalAñoFilter($inicio,$fechaFin,$lugar); //total cerdos-kilos del intervalo de fechas
		// print_r($total1);
		require_once('view/novedadesReportFilterView.php');
